...
| R-013 | Genuine AbstractEventLoop subclass | ✅ | python/cadeloop/loop.py |
...
| R-032 | AcceptEx pool + GetAcceptExSockaddrs + update-context | 🔶 primitive ✅, pool/addrs in M1 listener | iocp.rs |
| R-033 | DisconnectEx(TF_REUSE_SOCKET) + 4096-cap pool | ✅ | iocp.rs |
| R-034 | ConnectEx + bind-before-connect + update-context | ✅ | iocp.rs |
| R-035 | Gather WSASend ≤16 WSABUFs; corking | 🔶 gather ✅, corking with M1 transports | iocp.rs |
...
| R-050 | AbstractEventLoop surface + native fast paths | 🔶 scheduling ✅, I/O methods milestone-gated | coreloop.rs, loop.py |
| R-051 | Subprocess gated (default off) | ✅ gate + message | config.py, loop.py |
| R-052 | SIGINT/SIGBREAK handlers | 📅 M4 | loop.py (gated) |
| R-053 | 4-ary timer heap, tombstones, >50% compaction | ✅ (proptested) | timer.rs |
| R-054 | 128-batch dispatch, vectorcall, bound-method cache | 🔶 batching+vectorcall ✅; per-connection callback cache with M1 | ready.rs, handles.rs |
...
| R-057 | add_reader/add_writer emulation | 📅 M1/M4 | loop.py (gated) |
| R-058 | UDP endpoints | 📅 M4 | loop.py (gated) |
| R-059 | Native TLS (memory BIOs), SSLContext compat | 📅 M4 | loop.py (gated) |
...
| R-072 | Zero-copy memoryview via BufferedProtocol | 📅 M1 | buffers.rs (refcount hooks ready) |
| R-073 | Slot lifetime refcounts + poison | ✅ (proptested) | buffers.rs |
| R-074 | Write retention semantics (bytes zero-copy, bytearray copy) | 📅 M1 transports | — |
...
| R-122 | Edge-case matrix | 📅 M1 (soak harness ✅ for M0 surface) | tests/stress |
| R-123 | ASGI compliance + Starlette/FastAPI | 📅 M2 | — |
| R-124 | Interop smoke (aiohttp/asyncpg/aiofastnet) | 📅 M1/M4 | — |
| R-130 | Harness: warmups, medians, JSON baselines | ✅ | bench/harness |
| R-131 | bombardier/rewrk/custom client; two-machine authority | 📅 M1/M2 (documented everywhere loopback appears) | bench/*/README |
...
2. **Portable dev backend** (not in spec): non-Windows development/CI aid;
   never shipped (wheels remain cp311-win_amd64).
