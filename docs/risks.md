...
| `__del__`/GC re-entrancy inside state critical section | Implemented: graveyard protocol — no Python decref inside the cell; discarded tokens dropped outside (`timer.rs`/`reactor.rs`/`coreloop.rs`). |
